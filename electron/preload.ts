...
const api = {
  ...
  pauseUpdate: () => ipcRenderer.invoke('updates:pause'),
  resumeUpdate: () => ipcRenderer.invoke('updates:resume'),
  ...
  // build & setup
  buildDetect: (dir: string) => ipcRenderer.invoke('build:detect', dir),
  buildGetConfig: (projectPath: string) => ipcRenderer.invoke('build:getConfig', projectPath),
  buildSaveConfig: (config: unknown) => ipcRenderer.invoke('build:saveConfig', config),
  buildResetConfig: (projectPath: string) => ipcRenderer.invoke('build:resetConfig', projectPath),
  buildComputeExclusions: (dir: string) => ipcRenderer.invoke('build:computeExclusions', dir),
  buildPreflight: (config: unknown) => ipcRenderer.invoke('build:preflight', config),
  buildStart: (config: unknown) => ipcRenderer.invoke('build:start', config),
  buildCancel: (buildId: string) => ipcRenderer.invoke('build:cancel', buildId),
  buildRetryStage: (buildId: string, stageId?: string) => ipcRenderer.invoke('build:retryStage', buildId, stageId),
  buildGetState: (buildId: string) => ipcRenderer.invoke('build:getState', buildId),
  buildOpenOutput: (dir: string) => ipcRenderer.invoke('build:openOutput', dir),
  buildCopySummary: (text: string) => ipcRenderer.invoke('build:copySummary', text),
  buildRecentPaths: () => ipcRenderer.invoke('build:recentPaths'),
  buildEligibility: (projectPath: string) => ipcRenderer.invoke('build:eligibility', projectPath),
  buildEligibilityMany: (projectPaths: string[]) => ipcRenderer.invoke('build:eligibilityMany', projectPaths),
  onBuildLog: (cb: (...args: unknown[]) => void) => on('build:log', cb),
  onBuildStage: (cb: (...args: unknown[]) => void) => on('build:stage', cb),
  onBuildState: (cb: (...args: unknown[]) => void) => on('build:state', cb),
  // events
